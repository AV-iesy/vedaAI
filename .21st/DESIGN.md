<!-- Generated by 21st. Edit constraints and decisions in .21st/design.json. -->
# Project Design Context

Generated from project sources at 2026-08-26T08:19:20.824Z.

## Project

- Name: veda-ai
- Product type: Unknown
- Stack: nextjs, react
- Color mode: light
- Density: unknown

## Sources

- Tokens: app/globals.css
- Components: None detected
- Assets: public/assets/joyful-train-sky.png, public/assets/pixel-sunflower-field.png
- Instructions: None detected

## Components

- Installed: None detected
- Preferred primitives: None detected

## Tokens

### Colors

- `--muted`: `#716b89` (app/globals.css)
- `--lavender`: `#c9b8ff` (app/globals.css)
- `--pink`: `#ffb8d2` (app/globals.css)
- `--mint`: `#aee8cf` (app/globals.css)
- `--sky`: `#addff5` (app/globals.css)
- `--yellow`: `#ffe995` (app/globals.css)

### Shadows

- `--shadow`: `0 18px 54px rgba(38, 54, 45, 0.09)` (app/globals.css)

## Constraints

### Must

- None recorded

### Avoid

- None recorded

## Decisions

- Use a colorful pastel anime and pixel-art visual language with crisp plum outlines, offset shadows, and compact review density.
- Use an original wide pixel-art sunflower field, generated from the user's scenery and style references, as the active upload-screen background.
- Reserve the first viewport exclusively for scenery and place the complete upload workflow in the second full section.
- Use a plain VedaAI wordmark, a yellow pixel-style AI assistant button with a purple sparkle and pink hover state, and a centered cream serif title on the scenery page.
